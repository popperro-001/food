...
function calc() {
  ...
  function initLocalSettings(selector, activeClass) {
    //check values fromm localStorage and decorate accordingly calculator
  ...
  function getStaticInformation(selector, activeClass) {
    //function for 2 blocks: geneder and activity
  ...
  function getDynamicInformation(selector) {
    ...
    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) {
        //if user enters anything besides the digits
...
function cards() {
  ...
  class MenuCard {
    ...
    exchangeToUAH() {
      //transform price from usd to uah
...
function forms(formSelector, modalTimerId) {
  ...
  forms.forEach(item => {
    //assign function postData to each form on the page
    bindPostData(item);
  });
  ...
  function showThanksModal(message) {
    //create info modal after form was submitted
    ...
    setTimeout(() => {
      //bring back default settings for modal
      thanksModal.remove();
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  /* Modal */
  const modal = document.querySelector(modalSelector),
        btnOpenModal = document.querySelectorAll(triggerSelector); // btnCloseModal = document.querySelector('[data-close]'); this approach would not work with the dynamic created content
  ...
  modal.addEventListener('click', e => {
    //if user clicks anywhere besides the opened modal, modal should close
    if (e.target === modal || e.target.getAttribute('data-close') == '') {
      //addid getAttribute to close modal
      closeModal(modalSelector);
    }
  });
  document.addEventListener('keydown', e => {
    //if user press esc button on keyboard, modal should close
  ...
  function showModalByScroll() {
    //if user scrolled down till the end of the page modal will pop-up automatically
...
function slider(_ref) {
  ...
  for (let i = 0; i < slides.length; i++) {
    //create list item for each slide
  ...
  next.addEventListener('click', () => {
    if (offset == +parseInt(width) * (slides.length - 1)) {
      //if we reached the end of slider we return to the beginning, aslo we need to parseInt because width = string(ex '500px')
      offset = 0;
    } else {
  ...
  prev.addEventListener('click', () => {
    if (offset == 0) {
      //if we want to go beyond the beginning we return to the end of the slider
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    //set default value if we call function without arg
  ...
  tabsParent.addEventListener('click', event => {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      tabs.forEach((item, i) => {
        if (target == item) {
          //compare terget tab with tabs NodeList, to get the index = i, in order to put this index for tabContent
...
function getZero(num) {
  //function to add 0 if digit is less than 10
  if (num >= 0 && num < 10) {
    return `0${num}`;
  } else {
    return num;
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    //get difference between current date and arg date
  ...
  setClock(id, deadline); // console.log(new Date(Date.parse(deadline)));
}
...
const postData = async (url, data) => {
  //common function to handle fitch POST requests
  const result = await fetch(url, {
    //remember this is async code, error could be when result.json(), second when return, we need to use async before the function and await in the body of function
    method: "POST",
    headers: {
      //if we send JSON
...
const getResource = async url => {
  //common GET function to be reusable
  const result = await fetch(url);

  if (!result.ok) {
    //fetch does not handle 400 or 500 errors, so we need to handle it manually by catching the error
